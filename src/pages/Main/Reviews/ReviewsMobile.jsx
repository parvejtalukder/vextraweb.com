...
const ReviewCard = ({ img, name, username, body }) => (
  <figure className="flex flex-col items-center justify-center w-64 p-4 mx-4 rounded-xl hover:bg-white/5 transition-colors">
...
const ReviewCardMob = ({ img, name, username, body }) => (
  <figure className="flex flex-col items-center justify-center w-auto p-4 mx-4 rounded-xl hover:bg-white/5 transition-colors">
    <img src={img} alt={name} className="w-12 h-12 rounded-full mb-2" />
    <figcaption className="font-semibold text-white">{name}</figcaption>
    <p className="text-xs text-white/50 mb-2">{username}</p>
    <blockquote className="text-sm text-white/80 text-center">{body}</blockquote>
...
const ReviewsMobile = () => {
  return (
    <section className="py-5 text-white px-4">
